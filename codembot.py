...
async def scrape_anses(doc_number, update: Update):
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            # Esperar X segundo antes del primer intento
            if attempt == 0:
            ...
            while attempt <= max_attempts:
                ...
                # Extraer Descripción, Condición y Situación
                rows = table.find_all('tr')
                obrasocial, condicion, situacion = None, None, None
                ...
                if not obrasocial or not condicion or not situacion:
                    print("DEBUG: No se pudieron extraer obra social, condición o situación.")
                    if attempt == max_attempts:
                        print("DEBUG: Se alcanzó el máximo de intentos.")
                        return f"Error: No se pudieron extraer la obra social, condición o situación tras {max_attempts} intentos."
...
async def ping_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    msg = await update.message.reply_text("⏱️ Midiendo ping...")
    end_time = time.time()
    elapsed = int((end_time - start_time) * 1000)  # en milisegundos
    await msg.edit_text(f"🏓 Pong! Tiempo de respuesta: {elapsed} ms")
...
def main():
    ...
    application.add_handler(CommandHandler("ping", ping_command))  # 👈 este
